...
class TreatmentRecordProvider with ChangeNotifier {
  ...
  final Logger _logger = Logger('TreatmentRecordProvider');
  final Dio _dio = Dio();
  ...
  Future<TreatmentRecord?> fetchRecordById(String id, String token) async {
    try {
      ...
      if (response.statusCode == 200) {
        return TreatmentRecord.fromJson(response.data);
      } else {
